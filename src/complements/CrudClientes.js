...
function Crud_clientes() {
    ...
    client.info(nuevoCliente.correo, { catch_all: 0 })
      .then(response => {
        if (response.smtp_check) {
          ...
          Axios.post("https://backenddlytime-efgpffakcxe2d9e9.brazilsouth-01.azurewebsites.net/crudClientes/agregarCliente", nuevoCliente)
            .then((res) => {
              if (res.data.exists) {
  ...
  const guardarEdicion = () => {
    Axios.patch("https://backenddlytime-efgpffakcxe2d9e9.brazilsouth-01.azurewebsites.net/crudClientes/actualizarCliente", clienteEditado)
      .then((res) => {
        if (res.data.exists) {
